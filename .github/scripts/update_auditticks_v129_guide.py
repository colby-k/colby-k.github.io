...
text = text.replace('v1.2.28', 'v1.2.29')
text = text.replace(
...
old = '''      <section id="modes-position" class="atg-section atg-searchable" data-search-title="In-Cell Floating Append modes and position" data-search-category="Document Testing">
...
new = '''      <section id="modes-position" class="atg-section atg-searchable" data-search-title="In-Cell Floating Append modes position alignment Left Center Right" data-search-category="Document Testing">
...
if old not in text:
    raise SystemExit('Expected modes-position section was not found; guide was not modified.')
text = text.replace(old, new, 1)
marker = '          <details><summary>I expected Append to work in Floating mode.</summary>'
addition = '          <details><summary>Left, Center, or Right did not move a floating mark.</summary><div><p>The Position controls apply horizontal alignment to worksheet cells and set the default position for future <strong>in-cell</strong> marks. Floating marks are independent worksheet objects, so reposition them directly on the worksheet instead.</p></div></details>\n'
